...
export interface LanguageKnowledgeStore {
  ...
  acceptProposal(input: {
    ...
    registry: KnowledgeRegistryRecord;
  } | null>;
}
...
type DbProposal = {
  ...
  user_id: string;
  profile_record_id: string;
  base_registry_record_id: string;
  adaptation_plan_id: string;
  adaptation_plan_version: string;
  research_task_ref: string;
  operation: "create" | "extend";
  requirement_refs: string[];
  base_decision_id: string | null;
  base_decision_version: string | null;
  decision_id: string;
  decision_version: string;
  proposed_decision: DecisionProposal["decision"];
...
function mapProposal(row: DbProposal): DecisionProposalRecord {
  return {
    ...
    adaptationPlanRef: {
      id: row.adaptation_plan_id,
      version: row.adaptation_plan_version,
    },
    ...
    proposal: {
      ...
      ...(row.base_decision_id && row.base_decision_version
        ? {
            baseDecisionRef: {
              id: row.base_decision_id,
              version: row.base_decision_version,
            },
          }
        : {}),
...
class ProposalAcceptConflictError extends Error {
  constructor() {
    super("proposal_accept_conflict");
    this.name = "ProposalAcceptConflictError";
  }
}

export const languageKnowledgeStore: LanguageKnowledgeStore = {
  async createProfile(input) {
    const inserted = rows<DbProfile>(
      await getDb().execute(sql`
        ...
        ON CONFLICT (user_id, profile_id, version) DO NOTHING
        RETURNING *
      `),
    );
    if (inserted[0]) return mapProfile(inserted[0]);

    const existing = rows<DbProfile>(
      await getDb().execute(sql`
        SELECT * FROM language_knowledge_profiles
        WHERE user_id=${input.userId}
          AND profile_id=${input.profile.identity.profileId}
          AND version=${input.profile.version}
        LIMIT 1
      `),
    );
    if (!existing[0]) throw new Error("profile_persistence_failed");
    if (existing[0].content_sha256 !== input.contentSha256) {
      throw new Error("immutable_profile_version_conflict");
    }
    return mapProfile(existing[0]);
  },

  async findProfile(userId, profileRecordId) {
    const result = rows<DbProfile>(
      await getDb().execute(sql`
        SELECT * FROM language_knowledge_profiles
        WHERE id=${profileRecordId} AND user_id=${userId}
        LIMIT 1
      `),
    );
  ...
  async createRegistry(input) {
    ...
    const existing = rows<DbRegistry>(
      await getDb().execute(sql`
  ...
  async createProposal(input) {
    ...
    const existing = rows<DbProposal>(
      await getDb().execute(sql`
        SELECT * FROM language_decision_proposals
        WHERE user_id=${input.userId}
          AND base_registry_record_id=${input.baseRegistryRecordId}
          ...
          AND proposal_sha256=${input.proposalSha256}
        LIMIT 1
      `),
    );
  ...
  async acceptProposal(input) {
    try {
      return await getDb().transaction(async (tx) => {
        const registryRows = rows<DbRegistry>(
          await tx.execute(sql`
            ...
            WHERE proposal.id=${input.proposalId}
              AND proposal.user_id=${input.userId}
              AND proposal.status='pending_review'
              AND proposal.profile_record_id=${input.profileRecordId}
            ON CONFLICT (user_id, registry_id, version) DO NOTHING
            RETURNING *
          `),
        );
        ...
        const proposalRows = rows<DbProposal>(
          await tx.execute(sql`
            UPDATE language_decision_proposals
            SET status='accepted', review_note=${input.note},
              review_evidence_status=${input.evidenceReviewStatus},
              review_confidence=${input.confidence}, reviewed_at=now(),
              promoted_registry_record_id=${registryRow.id}
            WHERE id=${input.proposalId}
              AND user_id=${input.userId}
              AND status='pending_review'
            RETURNING *
        ...
        if (!proposalRows[0]) {
          throw new ProposalAcceptConflictError();
        }

        return {
          proposal: mapProposal(proposalRows[0]),
          registry: mapRegistry(registryRow),
        };
      });
    } catch (error) {
      if (error instanceof ProposalAcceptConflictError) return null;
      throw error;
    }
  },
};
